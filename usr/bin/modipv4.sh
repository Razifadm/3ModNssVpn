...
flow() {
  ...
  uci set turboacc.config.fullcone6='0'
  uci set turboacc.config.hw_wed='0'
  uci set turboacc.config.bbr_cca='1'
  uci commit turboacc >/dev/null 2>&1

  logger -t TTL64 "MODE: TurboAcc (TTL removed, Turbo + turboacc config set)"
  restart_services
}
...
nss() {
  ...
  logger -t TTL64 "MODE: NSS (TTL removed, QCA NSS ECM enabled)"
  restart_services
}
...
vpn() {
  ...
  # Set turboacc config (sama macam flow)
  uci set turboacc.config.sw_flow='1'
  uci set turboacc.config.hw_flow='1'
  uci set turboacc.config.sfe_flow='1'
  uci set turboacc.config.fullcone_nat='1'
  ...
  logger -t TTL64 "MODE: VPN (TTL active, Turbo + turboacc config set)"
  restart_services
}
